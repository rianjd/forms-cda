...
<div class="container">
    ...
    @if ($data['tipo'] == 'coreForm')<div class="d-flex align-items-center p-3 my-3 text-white-50 bg-secondary rounded shadow"><img class="mr-3" id="corebusiness_1200x300" src={{"https://github.com/rianjd/forms-cda/blob/master/public/images/corebusiness_1200x300.png?raw=true"}} width="150" height="40" style="display:block;" alt=""> @endif
    ...
    <div class="my-3 p-3 bg-white rounded shadow-sm">
        <h6 class="border-bottom border-gray pb-4 mb-0">Nova solicitação: </h6>
        ...
        @if (isset($data['cpf']))<div class="media text-muted pt-4">
            <div class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray"><p><strong>CPF: </strong>{{$data['cpf'] }}</p></div></div>@endif
        @if (isset($data['setor']))<div class="media text-muted pt-4">
            <div class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray"><p><strong>Setor: </strong>{{$data['setor'] }}</p></div></div>@endif
        @if (isset($data['filial']))<div class="media text-muted pt-4">
            <div class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray"><p><strong>Filial: </strong>{{$data['filial'] }}</p></div></div>@endif
            @if (isset($data['user']))<div class="media text-muted pt-4">
            <div class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray"><p><strong>Filial: </strong>{{$data['user'] }}</p></div></div>@endif
        @if (isset($data['inDJ']))<div class="media text-muted pt-4">
            <div class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray"><p><strong>Incluir no DJ: </strong>{{$data['inDJ'] }}</p></div></div>@endif
        @if (isset($data['msg']))<div class="media text-muted pt-4">
            <div class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray"><p><strong>Observações: </strong>{{$data['msg'] }}</p></div></div>@endif
